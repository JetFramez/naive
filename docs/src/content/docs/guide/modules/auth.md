...
`@jetframez/naive/auth` is deliberately minimal: strategies, sessions, JWTs, opaque tokens with rotation, and password hashing. There is no OAuth, no magic links, no 2FA, no email verification, no password reset, and no authorization model — build those on top with `currentUser()` and your own logic.

```ts
import { createAuth, cookieSession, jwt, apiKey } from "@jetframez/naive/auth";
...
```ts
import { argon2 } from "@jetframez/naive/auth";
...
```ts
import { currentUser, requireUser } from "@jetframez/naive/auth";
