...
`notio/rate-limit` limits requests by a key. In one process it runs in memory; scaled out across processes or machines, hand it your Redis client instead and every decision becomes one atomic Lua script on the server.

<Tabs>
<TabItem label="memory — one process">
```ts
import { rateLimit } from "notio/rate-limit";
