...
naive logs through pino, behind a small `Logger` interface: `trace`, `debug`, `info`, `warn`, `error`, `fatal`, `child(fields)`, `isLevelEnabled(level)` and a `level` property. Each method takes either a message or a fields object followed by a message, as pino does.

```ts
import { log } from "@jetframez/naive";
...
```ts
import { currentCtx, requireCtx } from "@jetframez/naive";
