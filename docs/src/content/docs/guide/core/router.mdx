...
`.query()`, `.body()`, `.headers()` and `.response()` accept any [Standard Schema](https://standardschema.dev) value. Zod, Valibot and ArkType all work; naive has no dependency on any of them.
...
- The body is whatever the body parser put on `req.body`. `createApp` always installs a JSON parser; urlencoded parsing is off by default (`body.urlencoded` in [createApp](../app/#options)). On bare Express, install `express.json()` yourself — see [Using naive in an existing Express app](../../getting-started/existing-express/).
