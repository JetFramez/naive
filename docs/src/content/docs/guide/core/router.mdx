...
This page covers the type layer: declaring params, schemas, statics and redirects, and what the router records about each route. For middleware and narrowing, see [Middleware](/guide/core/middleware). For what a handler's return value turns into, see [Responses](/guide/core/responses).
...
- The body is whatever the body parser put on `req.body`. `createApp` always installs a JSON parser; urlencoded parsing is off by default (`body.urlencoded` in [createApp](/guide/core/app#options)). On bare Express, install `express.json()` yourself — see [Using notio in an existing Express app](/guide/getting-started/existing-express).
