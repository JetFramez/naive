...
`bearer()` returns the bearer token or `undefined`. `accepts(...types)` wraps `req.accepts`. `bind(fields)` adds fields to every subsequent `ctx.log` line for the rest of the request — see [Logging: ambient context](/guide/core/logging#ambient-context).

`ctx.status(code)` and `ctx.set(name, value)` are chainable mutators for plain return values. When a handler returns a descriptor, the descriptor's status and headers win. See [Responses](/guide/core/responses) for the full set of return conventions, including `ctx.json()` and the other descriptor factories.
...
For fields a specific middleware adds, prefer `Middleware<{ user: User }>`, which narrows `ctx` for everything after it in the chain instead of making the field optional everywhere — see [Middleware](/guide/core/middleware#ctx-middleware-and-narrowing).
