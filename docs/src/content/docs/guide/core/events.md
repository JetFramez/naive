...
```ts
import { createEvents } from "@jetframez/naive";
...
There is no outbox, queue or persistence; events are lost if the process dies before listeners run. Use `emitAndWait` when the side effect must complete before the response — see [What naive does not do](../../about/scope/) for the reasoning.
