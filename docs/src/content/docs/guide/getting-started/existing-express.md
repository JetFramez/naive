---
title: Using naive in an existing Express app
---

`createApp` wires everything for you, but nothing requires it. Every piece works standalone on an `express()` app you already have, so you can adopt naive one router at a time.
...
import { context, Router } from "@jetframez/naive";

const app = express();
app.use(express.json()); // naive does not install body parsers for you here
...
```ts
import { errorHandler, notFound } from "@jetframez/naive";
...
```ts
import { hooks } from "@jetframez/naive";
...
## Mixing plain Express routes and naive routers
...
app.use(orders); // a naive Router
app.use("/v2", accounts.express()); // the compiled express.Router, if you want the raw instance
```

A plain Express route that throws still reaches the naive error handler if one is registered last — it just never passes through a router's `onError` hooks, since those only see errors from routes the router itself handles.
