...
Full detail: [createApp](/guide/core/app).
...
A thrown error at any point after step 1 skips straight to the router's `onError` hooks, then the app's, then the error handler — see [Errors](/guide/core/errors) and [Hooks](/guide/core/hooks).
...
Full detail: [Router](/guide/core/router), [Middleware](/guide/core/middleware).
...
The router owns the send step — you never call `res.send()` yourself in ordinary use. A plain object becomes JSON; `undefined` becomes 204; a descriptor from `ctx.json()`, `ctx.redirect()` and friends becomes exactly what it describes. See [Responses](/guide/core/responses).
...
- [Using notio in an existing Express app](/guide/getting-started/existing-express) if you are not starting from `createApp`.
- [Testing](/guide/guides/testing) for what this ordering means for driving requests in tests.
- [Production](/guide/guides/production) for what changes between `development` and `production`.
