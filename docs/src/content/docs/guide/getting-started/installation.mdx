...
Express itself ships inside `notio` — there is nothing else to install for the core. Zod is one option among three; swap it for `valibot` or `arktype` and nothing else in this guide changes.
...
| Import | What it adds |
|---|---|
| `notio` | `createApp`, `Router`, `Ctx`, errors, hooks, logging, config, events, cache |
| `notio/auth` | sessions, JWTs, opaque tokens, password hashing |
| `notio/upload` | multipart file uploads |
| `notio/rate-limit` | request rate limiting |
| `notio/openapi` | OpenAPI 3.1 generation and a docs UI |
