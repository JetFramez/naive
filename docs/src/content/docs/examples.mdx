...
`notio/auth` wired to a real database: an `AuthAdapter` implemented on Drizzle ORM against SQLite, backing signup, login, `GET /me`, and logout. Read [Auth](../guide/modules/auth/) alongside this for what each piece means.
