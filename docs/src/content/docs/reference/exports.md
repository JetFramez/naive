...
| `BadRequest`, `Unauthorized`, `Forbidden`, `NotFound`, `MethodNotAllowed`, `Conflict`, `Gone`, `PayloadTooLarge`, `Unprocessable`, `TooManyRequests`, `Internal`, `ServiceUnavailable` | The built-in `HttpError` subclasses — full table in [error codes](/reference/error-codes) |
...
| `parseDuration`, `parseBytes` | Duration and byte-size string parsing — [reference](/reference/durations-and-sizes) |
